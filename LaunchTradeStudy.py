...
# mission inputs: payload mass and target orbit for the trade study
PAYLOAD_MASS_KG = 250.0
TARGET_ORBIT    = "SSO (~550 km)"

# vehicle dataset: pricing, capacity, and mission characteristics for each option
vehicles = {
    "SpaceX Transporter\n(Direct Rideshare)": {
        "type":               "rideshare",
        "max_payload_kg":     1000,
        ...
        "reliability":        0.985,
        "orbit_flexibility":  "Low",
        "schedule_control":   "Low",
        "integration_lead_wks": 26,
    ...
    "Exolaunch\n(Rideshare Integrator)": {
        ...
        "price_base_mass_kg": 50,
        "price_per_kg":       7_000,
        "integrator_margin":  0.18,
        "deployer_fee":       85_000,
        "launch_cadence_days":90,
        "reliability":        0.985,
        "orbit_flexibility":  "Medium",
        "schedule_control":   "Low-Medium",
        "integration_lead_wks": 20,
    ...
    "Rocket Lab\nElectron": {
        "type":               "dedicated",
        "max_payload_kg":     300,
        "price_per_launch":   7_500_000,
        "launch_cadence_days":30,
        "reliability":        0.952,
        "orbit_flexibility":  "High",
        "schedule_control":   "High",
        "integration_lead_wks": 12,
...
def compute_cost(vehicle_name, vehicle, payload_kg):
    """Return cost and feasibility for one launch option."""
    vtype = vehicle["type"]
...
# scoring weights used to compare launch options on multiple criteria
WEIGHTS = {
    "cost_score":        0.40,
    "schedule_score":    0.20,
    "orbit_score":       0.20,
...
def score_vehicles(results):
    """Compute a normalized weighted score for each feasible flight option."""
...
# evaluate each vehicle with the mission payload and collect metrics
results = {}
...
# build sensitivity curve by sweeping payload mass from 50 to 500 kg
...
# create summary plots for cost, cost sensitivity, unit cost, and score breakdown
...
ax1.set_title(f"Total Mission Cost @ {PAYLOAD_MASS_KG} kg", fontsize=11, pad=10, fontfamily="monospace")
...
for name, costs in sensitivity.items():
    ...
    if valid_m:
        ...
        total_at_selected, _, feasible, _ = compute_cost(name, vehicles[name], PAYLOAD_MASS_KG)
        if feasible:
            ax2.scatter([PAYLOAD_MASS_KG], [total_at_selected / 1_000_000],
                        color=results[name]["color"], s=60, zorder=5)

ax2.axvline(PAYLOAD_MASS_KG, color="#888780", linestyle="--", linewidth=1, alpha=0.6,
            label=f"Your payload ({PAYLOAD_MASS_KG} kg)")
...
ax3.set_title(f"Cost per Kilogram @ {PAYLOAD_MASS_KG} kg", fontsize=11, pad=10, fontfamily="monospace")
